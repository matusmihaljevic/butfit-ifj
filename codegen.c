...
void generate_impilict_retyping(ASTNode* binary_op_node){
    ...
    else if(binary_op_node->retype_flag == INT_TO_FLOAT_op2){
        DString_concat(&Output,"POPS GF@GF_RESULT\nINT2FLOATS\nPUSHS GF@GF_RESULT\n",NULL);
    }
    else if(binary_op_node->retype_flag == FLOAT_TO_INT){
...
void generate_while_loop(ASTNode* while_node){
    ...
    DString_concat(&Output,"LABEL $while_",&curr_id,"\n",NULL);
    ...
    DString_concat(&Output,"JUMP $while_",&curr_id,"\n",NULL);
    DString_concat(&Output,"LABEL $break_",&curr_id,"\n",NULL);
}
