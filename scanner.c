...
static bool is_internal_function(const char *function_name) {
    for (int i = 0; i < INTERN_FUNCS_CNT; i++) {
        if (strcmp(function_name, intern_functions[i]) == 0) {
            return true;
        }
    }
    return false;
...
int get_token(Token *token) {
        ...
        switch (state) {
            case STATE_INITIAL:
                switch (c) {
                    ...
                    default:
                        if (isalpha(c) || c == '_') {
							...
							fprintf(stdout, "Undefined character at line: %u\n", line);
							DString_free(&string);
				...
				case STATE_BACKSLASH:
					token->type = TOKEN_TYPE_BACKSLASH;
					token_complete = true;
					ungetc(c, source_file);
					break;
				case STATE_EOL:
				...
				case STATE_IDENTIFIER:
					if(isalpha(c) || isdigit(c) || c == '_') {
						...
						if (is_internal_function(string.data))
	...
	if(token->type == TOKEN_TYPE_IDENTIFIER || token->type == TOKEN_TYPE_INTERN || token->type == TOKEN_TYPE_STRING) {
