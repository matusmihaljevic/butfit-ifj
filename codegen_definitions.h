...
#define INTERN_FN_LENGHT DString_concat(&Output,"STRLEN GF@GF_RESULT string@",                   \
                                        built_in_fn_node->left->left->lexeme,                    \
                                        "\nPUSHS GF@GF_RESULT\n",NULL);                         

#define INTERN_FN_CONCAT DString_concat(&Output,"CONCAT GF@GF_RESULT LF@",                        \
                                        built_in_fn_node->left->left->lexeme,                     \
                                        " LF@",built_in_fn_node->left->right->left->lexeme,"\n",  \
                                        "PUSHS GF@GF_RESULT\n",NULL);                           

#define INTERN_FN_STRCMP char* curr_id = id++;                                                    \
                        DString_concat(&Output,"JUMPIFEQS $eq_",&curr_id,                         \
                                        "\nLTS\nPUSHS bool@false\nJUMPIFEQS $greater_",&curr_id,  \
                                        "\nPUSHS int@-1\nLABEL $greater_",&curr_id,               \
                                        "\nPUSHS int@1\nJUMP $end_",&curr_id,                     \
                                        "\nLABEL $eq_",&curr_id,                                  \
                                        "\nPUSHS int@0\nLABEL $end_",&curr_id,"\n",NULL);         
...
#define INTERN_FN_SUBSTR    char* curr_id = id++;                                    \
                            DString_concat(&Output,"POPS GF@GF_OPERAND2\n"           \
                                        ...
                                        "GT GF@GF_RESULT GF@GF_RESULT GF@GF_OPERAND2\n"              \
                                        "JUMPIFEQ $ret_nil_",&curr_id," GF@GF_RESULT bool@true\n"    \
                                        "LT GF@GF_RESULT GF@GF_OPERAND1 int@0\n"     \
                                        "JUMPIFEQ $ret_nil_",&curr_id," GF@GF_RESULT bool@true\n"    \
                                        "POPS GF@GF_SUBSTR\n"                  \
                                        "STRLEN GF@GF_RESULT GF@GF_SUBSTR"     \
                                        "LT GF@GF_RESULT GF@GF_RESULT GF@GF_OPERAND1\n"              \
                                        "NOT GF@GF_RESULT\n"                                         \
                                        "JUMPIFEQ $ret_nil_",&curr_id," GF@GF_RESULT bool@true\n"    \
                                        "SUB GF@GF_OPERAND2 int@1\n"                    \
                                        "MOVE GF@GF_SBSTR_O string@\n"                  \
                                        "LABEL $while_substr_",&curr_id,"\n"            \
                                        "JUMPIFEQ $end_while_substr",&curr_id,"GF@GF_OPERAND1 GF@GF_OPERAND2\n"    \
                                        "GETCHAR GF@GF_RESULT GF@GF_SUBSTR GF@GF_OPERAND1\n"                       \
                                        "CONCAT GF@GF_SBSTR_O GF@GF_SBSTR_O GF@GF_RESULT\n"                        \
                                        "ADD GF@GF_OPERAND1 int@1\n"                    \
...
#define INTERN_FN_ORD   DString_concat(&Output,"POPS GF@GF_OPERAND1\nPOPS GF@GF_RESULT\n",        \
                                       "GETCHAR GF@GF_RESULT GF@GF_RESULT GF@GF_OPERAND\n",       \
                                       "PUSHS GF@GF_RESULT\n",NULL);
